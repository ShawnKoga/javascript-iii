...
let totalPopulation = populations.reduce(function(popSum, popVal){
    return popSum + popVal;
})
...
let myStrongest = monstersInYourPocket.filter(function(strongMon){
    return strongMon.CP > 200;
    }
);
...
let ordersTotal = orders.map(function(cost){
    return cost.price * (1 + cost.tax)
});
...
let bobsTotal = purchases.reduce(function(who, cost){
    var bobsSum = 0;
    if(who[owner] === "Bob"){
        bobsSum = bobsSum + who[cost]
    }return bobsSum;
});
